...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    _LOGGER.info("Smart Shopping: JS served at %s and %s", CARD_URL, SUMMARY_URL)

    # Verify the JS files are actually present
    here = Path(__file__).parent
    for fname in ("smart-shopping-card.js", "smart-shopping-summary-card.js"):
        if not (here / fname).exists():
            _LOGGER.error(
                "Smart Shopping: %s not found in %s — card will not work!", fname, here
            )

    # Auto-register with Lovelace (works for most HA setups)
    try:
        from homeassistant.components.frontend import add_extra_js_url
        add_extra_js_url(hass, CARD_URL)
        add_extra_js_url(hass, SUMMARY_URL)
        _LOGGER.info("Smart Shopping: Lovelace resources registered automatically")
    except Exception as err:
        _LOGGER.warning(
            "Smart Shopping: Could not auto-register Lovelace resource (%s). "
            "Add these manually in Settings → Dashboards → Resources: %s  %s",
