...
const SUMMARY_STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── NEXT ITEMS — horizontal tile grid ── */
  .next-items { display:grid; grid-template-columns:repeat(auto-fill, minmax(110px, 1fr)); gap:8px; }
  .next-item  {
    background:rgba(255,255,255,.04); border:1px solid var(--ss-border);
    border-radius:12px; display:flex; flex-direction:column; overflow:hidden; transition:all .2s;
  }
  .next-item:hover { border-color:rgba(0,212,170,.4); }
  .next-item-thumb {
    width:100%; aspect-ratio:1/1; background:rgba(255,255,255,.05);
    display:flex; align-items:center; justify-content:center; font-size:28px; overflow:hidden;
  }
  .next-item-thumb img { width:100%; height:100%; object-fit:cover; }
  .next-item-body { padding:6px 7px 4px; flex:1; min-width:0; }
  .next-item-name { font-size:11px; font-weight:600; color:var(--ss-text); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .next-item-meta { font-size:10px; color:var(--ss-muted); margin-top:1px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .next-item-buy  {
    width:100%; border:none; background:linear-gradient(135deg, var(--ss-primary), #00a884);
    color:#000; font-size:10px; font-weight:800; padding:6px 4px;
    cursor:pointer; transition:filter .2s; letter-spacing:.2px;
  }
  .next-item-buy:hover { filter:brightness(1.15); }
  ...
  .ss-modal-overlay {
    ...
    padding:16px; animation:ssFadeIn .2s ease;
  }
  ...
  .ss-modal {
    ...
    box-shadow:0 24px 80px rgba(0,0,0,.5); animation:ssSlideUp .25s ease;
  }
  .ss-modal-header {
    padding:20px 20px 0; display:flex; align-items:center;
    justify-content:space-between; margin-bottom:16px;
  }
  .ss-modal-title { font-size:18px; font-weight:700; color:#fff; }
  .ss-modal-close {
    ...
    justify-content:center; font-size:18px; color:var(--ss-text); transition:all .2s;
  }
  .ss-modal-close:hover { background:rgba(255,107,107,.2); color:var(--ss-accent); }
  .ss-modal-body { padding:0 20px 20px; }

  .ss-form-group { margin-bottom:14px; }
  .ss-form-label {
    display:block; font-size:12px; font-weight:600; color:var(--ss-muted);
    letter-spacing:.5px; margin-bottom:6px; text-transform:uppercase;
  }
  .ss-form-input, .ss-form-select {
    width:100%; background:rgba(255,255,255,.05); border:1px solid var(--ss-border);
    border-radius:10px; padding:10px 14px; font-size:14px; color:var(--ss-text);
    outline:none; transition:border-color .2s; font-family:inherit; appearance:none;
  }
  .ss-form-input::placeholder { color:var(--ss-muted); }
  .ss-form-input:focus, .ss-form-select:focus { border-color:var(--ss-primary); }
  .ss-form-select option { background:#1a1a2e; }
  .ss-form-row { display:grid; grid-template-columns:1fr 1fr; gap:10px; }

  .ss-img-row { display:flex; gap:10px; align-items:center; }
  .ss-img-preview { width:60px; height:60px; border-radius:10px; object-fit:cover; border:1px solid var(--ss-border); flex-shrink:0; }
  .ss-img-placeholder {
    width:60px; height:60px; border-radius:10px; border:1px dashed var(--ss-border);
    display:flex; align-items:center; justify-content:center; font-size:24px;
    color:var(--ss-muted); flex-shrink:0;
  }

  .ss-btn-primary {
    width:100%; background:linear-gradient(135deg,var(--ss-primary),#00a884); border:none;
    border-radius:10px; padding:12px; font-size:15px; font-weight:600; color:#000;
    cursor:pointer; transition:all .2s; margin-top:4px;
  }
  .ss-btn-primary:hover { transform:translateY(-1px); box-shadow:0 4px 20px rgba(0,212,170,.3); }
...
class SmartShoppingSummaryCard extends HTMLElement {
  ...
  _openEditModal(item) {
    this._editForm = { ...item };
    this._modalDiv.innerHTML = this._buildEditModal();
    this._bindEditModal();
  }
  ...
  _buildNextItemsSection(maxItems) {
    ...
    const rows = unchecked.map(item => {
      ...
      const thumb   = item.image_url
        ...
        : _icon(catInfo.icon, 28, catInfo.color);
      const meta    = [
        item.store || "",
        qtyLbl,
      ].filter(Boolean).join(" · ");
      return `
        <div class="next-item">
          <div class="next-item-thumb">${thumb}</div>
          <div class="next-item-body" data-edit="${item.name}" style="cursor:pointer" title="Tap to edit">
            <div class="next-item-name">${item.name}</div>
            ${meta ? `<div class="next-item-meta">${meta}</div>` : ""}
          </div>
          <button class="next-item-buy" data-buy="${item.name}">✓ Got it</button>
        </div>`;
    }).join("");
  ...
  _buildEditModal() {
    ...
    return `
      <div class="ss-modal-overlay" id="ss-overlay">
        <div class="ss-modal">
          <div class="ss-modal-header">
            <div class="ss-modal-title">✏️ Edit Item</div>
            <button class="ss-modal-close" id="ss-modal-close">✕</button>
          </div>
          <div class="ss-modal-body">
            <div class="ss-form-group">
              <label class="ss-form-label">Item Name</label>
              <input class="ss-form-input" id="ss-f-name" type="text" value="${f.name||""}">
            </div>
            <div class="ss-form-row">
              <div class="ss-form-group">
                <label class="ss-form-label">Quantity</label>
                <input class="ss-form-input" id="ss-f-qty" type="number" min="1" value="${f.quantity||1}">
              </div>
              <div class="ss-form-group">
                <label class="ss-form-label">Unit</label>
                <input class="ss-form-input" id="ss-f-unit" type="text" placeholder="kg, L, pcs…" value="${f.unit||""}">
              </div>
            </div>
            <div class="ss-form-group">
              <label class="ss-form-label">Category</label>
              <select class="ss-form-select" id="ss-f-cat">
                ${cats.map(c => `<option value="${c.name}" ${f.category===c.name?"selected":""}>${c.icon||"📦"} ${c.name}</option>`).join("")}
              </select>
            </div>
            <div class="ss-form-group">
              <label class="ss-form-label">Store</label>
              <select class="ss-form-select" id="ss-f-store">
                <option value="">Any store</option>
                ${stores.map(s => `<option value="${s.name}" ${f.store===s.name?"selected":""}>${s.icon||"🛒"} ${s.name}</option>`).join("")}
              </select>
            </div>
            <div class="ss-form-group">
              <label class="ss-form-label">Image URL (optional)</label>
              <div class="ss-img-row">
                ${f.image_url && _isUrl(f.image_url)
                  ? `<img id="ss-img-preview" class="ss-img-preview" src="${f.image_url}" alt="preview">`
                  : `<div id="ss-img-preview" class="ss-img-placeholder">🖼</div>`}
                <input class="ss-form-input" id="ss-f-image" type="text" placeholder="https://… or /local/…" value="${f.image_url||""}">
              </div>
            </div>
            <div class="ss-form-group">
              <label class="ss-form-label">Notes (optional)</label>
              <input class="ss-form-input" id="ss-f-notes" type="text" value="${f.notes||""}">
            </div>
            <button class="ss-btn-primary" id="ss-f-submit">Save Changes</button>
          </div>
        </div>
      </div>`;
  }
  ...
  _bindEditModal() {
    ...
    $("ss-modal-close")?.addEventListener("click", () => this._closeModal());
    $("ss-overlay")?.addEventListener("click", e => { if (e.target.id === "ss-overlay") this._closeModal(); });

    $("ss-f-name")?.addEventListener("input",  e => { f.name     = e.target.value; });
    $("ss-f-qty")?.addEventListener("input",   e => { f.quantity = parseInt(e.target.value)||1; });
    $("ss-f-unit")?.addEventListener("input",  e => { f.unit     = e.target.value; });
    $("ss-f-notes")?.addEventListener("input", e => { f.notes    = e.target.value; });
    $("ss-f-cat")?.addEventListener("change",  e => { f.category = e.target.value; });
    $("ss-f-store")?.addEventListener("change",e => { f.store    = e.target.value; });

    $("ss-f-image")?.addEventListener("input", e => {
      const val = e.target.value;
      f.image_url = val;
      const preview = sr.getElementById("ss-img-preview");
      if (!preview) return;
      if (_isUrl(val)) {
        if (preview.tagName === "IMG") {
          ...
          img.id = "ss-img-preview"; img.className = "ss-img-preview"; img.src = val; img.alt = "preview";
    ...
    $("ss-f-submit")?.addEventListener("click", () => {
      const name = (sr.getElementById("ss-f-name")?.value || f.name).trim();
      if (!name) return;
      ...
      this._closeModal();
      this._render();
      if (this._hass) this._hass.callService("smart_shopping", "update_items", { items: this._state.items });
    });
  }
  ...
  _bindEvents() {
    ...
    // "Got it" — mark purchased AND remove in one tap
    sr.querySelectorAll("[data-buy]").forEach(el => {
      el.addEventListener("click", e => {
        e.stopPropagation();
        const name = el.dataset.buy;
        const tile = el.closest(".next-item");
        if (tile) { tile.style.transition="transform .18s,opacity .18s"; tile.style.transform="scale(.85)"; tile.style.opacity="0"; }
        setTimeout(() => {
          if (!this._hass) return;
          this._hass.callService("smart_shopping", "check_item",  { name });
          this._hass.callService("smart_shopping", "remove_item", { name });
